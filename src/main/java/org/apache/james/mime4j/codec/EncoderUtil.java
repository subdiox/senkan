...
public class EncoderUtil {
    ...
    private static boolean isDotAtomText(String str) {
        char prev = FilenameUtils.EXTENSION_SEPARATOR;
        int length = str.length();
        if (length == 0) {
            return false;
        }
        ...
        while (idx < length) {
            char ch = str.charAt(idx);
            if (ch == FilenameUtils.EXTENSION_SEPARATOR) {
                if (prev == FilenameUtils.EXTENSION_SEPARATOR || idx == length - 1) {
                    return false;
                }
