...
public class Header implements Iterable<Field> {
    ...
    public int removeFields(String name) {
        List<Field> removed = (List) this.fieldMap.remove(name.toLowerCase());
        if (removed == null || removed.isEmpty()) {
            return 0;
        }
        Iterator<Field> iterator = this.fields.iterator();
        while (iterator.hasNext()) {
            if (((Field) iterator.next()).getName().equalsIgnoreCase(name)) {
                iterator.remove();
            }
        }
        return removed.size();
    ...
    public void setField(Field field) {
        List<Field> l = (List) this.fieldMap.get(field.getName().toLowerCase());
        if (l == null || l.isEmpty()) {
            addField(field);
            return;
        }
        ...
        Iterator<Field> iterator = this.fields.iterator();
        while (iterator.hasNext()) {
            if (((Field) iterator.next()).getName().equalsIgnoreCase(field.getName())) {
                iterator.remove();
