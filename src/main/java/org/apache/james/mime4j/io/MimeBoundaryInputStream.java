...
public class MimeBoundaryInputStream extends LineReaderInputStream {
    ...
    public MimeBoundaryInputStream(BufferedLineReaderInputStream inbuffer, String boundary) throws IOException {
        ...
        this.boundary[0] = (byte) 45;
        this.boundary[1] = (byte) 45;
        for (int i = 0; i < boundary.length(); i++) {
            byte ch = (byte) boundary.charAt(i);
            if (ch == (byte) 13 || ch == (byte) 10) {
    ...
    private int fillBuffer() throws IOException {
        ...
        while (i > 0 && this.buffer.charAt(i - 1) != (byte) 10) {
    ...
    private void calculateBoundaryLen() throws IOException {
        ...
        int len = this.limit - this.buffer.pos();
        if (len > 0 && this.buffer.charAt(this.limit - 1) == (byte) 10) {
            this.boundaryLen++;
            this.limit--;
        }
        if (len > 1 && this.buffer.charAt(this.limit - 1) == (byte) 13) {
            this.boundaryLen++;
            this.limit--;
        }
    }
