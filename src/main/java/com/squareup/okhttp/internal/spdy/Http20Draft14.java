...
public final class Http20Draft14 implements Variant {
    ...
    static final class ContinuationSource implements Source {
        ...
        private void readContinuationHeader() throws IOException {
            ...
            int access$200 = Http20Draft14.readMedium(this.source);
            this.left = access$200;
            this.length = access$200;
            byte type = (byte) (this.source.readByte() & 255);
            this.flags = (byte) (this.source.readByte() & 255);
            if (Internal.logger.isLoggable(Level.FINE)) {
                Internal.logger.fine(FrameLogger.formatHeader(true, this.streamId, this.length, type, this.flags));
            }
            this.streamId = this.source.readInt() & Integer.MAX_VALUE;
            if (type != Http20Draft14.TYPE_CONTINUATION) {
                throw Http20Draft14.ioException("%s != TYPE_CONTINUATION", Byte.valueOf(type));
        ...
        static String formatFlags(byte type, byte flags) {
            ...
            switch (type) {
                ...
                default:
                    ...
                    if (type != Http20Draft14.TYPE_PUSH_PROMISE || (flags & 4) == 0) {
    ...
    static final class Reader implements FrameReader {
        ...
        public void readConnectionPreface() throws IOException {
            if (!this.client) {
                ByteString connectionPreface = this.source.readByteString((long) Http20Draft14.CONNECTION_PREFACE.size());
                if (Internal.logger.isLoggable(Level.FINE)) {
                    Internal.logger.fine(String.format("<< CONNECTION %s", new Object[]{connectionPreface.hex()}));
                }
                if (!Http20Draft14.CONNECTION_PREFACE.equals(connectionPreface)) {
                    throw Http20Draft14.ioException("Expected a connection header but was %s", connectionPreface.utf8());
                }
            }
        }
        ...
        public boolean nextFrame(Handler handler) throws IOException {
            try {
                ...
                int length = Http20Draft14.readMedium(this.source);
                if (length < 0 || length > 16384) {
                    throw Http20Draft14.ioException("FRAME_SIZE_ERROR: %s", Integer.valueOf(length));
                }
                byte type = (byte) (this.source.readByte() & 255);
                byte flags = (byte) (this.source.readByte() & 255);
                int streamId = this.source.readInt() & Integer.MAX_VALUE;
                if (Internal.logger.isLoggable(Level.FINE)) {
                    Internal.logger.fine(FrameLogger.formatHeader(true, streamId, length, type, flags));
                }
                switch (type) {
                    case (byte) 0:
        ...
        private void readHeaders(Handler handler, int length, byte flags, int streamId) throws IOException {
            ...
            handler.headers(false, endStream, streamId, -1, readHeaderBlock(Http20Draft14.lengthWithoutPadding(length, flags, padding), padding, flags, streamId), HeadersMode.HTTP_20_HEADERS);
        }
        ...
        private void readData(Handler handler, int length, byte flags, int streamId) throws IOException {
            ...
            handler.data(inFinished, streamId, this.source, Http20Draft14.lengthWithoutPadding(length, flags, padding));
        ...
        private void readPriority(Handler handler, int length, byte flags, int streamId) throws IOException {
            if (length != 5) {
                throw Http20Draft14.ioException("TYPE_PRIORITY length: %d != 5", Integer.valueOf(length));
            } else if (streamId == 0) {
        ...
        private void readRstStream(Handler handler, int length, byte flags, int streamId) throws IOException {
            if (length != 4) {
                throw Http20Draft14.ioException("TYPE_RST_STREAM length: %d != 4", Integer.valueOf(length));
            } else if (streamId == 0) {
                ...
                if (errorCode == null) {
                    throw Http20Draft14.ioException("TYPE_RST_STREAM unexpected error code: %d", Integer.valueOf(errorCodeInt));
                } else {
        ...
        private void readSettings(Handler handler, int length, byte flags, int streamId) throws IOException {
            if (streamId != 0) {
                ...
                throw Http20Draft14.ioException("TYPE_SETTINGS length %% 6 != 0: %s", Integer.valueOf(length));
            } else {
                ...
                for (int i = 0; i < length; i += 6) {
                    ...
                    switch (id) {
                        ...
                        case (short) 5:
                            ...
                            throw Http20Draft14.ioException("PROTOCOL_ERROR SETTINGS_MAX_FRAME_SIZE: %s", Integer.valueOf(value));
                        default:
                            throw Http20Draft14.ioException("PROTOCOL_ERROR invalid settings id: %s", Short.valueOf(id));
                    }
        ...
        private void readPushPromise(Handler handler, int length, byte flags, int streamId) throws IOException {
            ...
            handler.pushPromise(streamId, this.source.readInt() & Integer.MAX_VALUE, readHeaderBlock(Http20Draft14.lengthWithoutPadding(length - 4, flags, padding), padding, flags, streamId));
        }

        private void readPing(Handler handler, int length, byte flags, int streamId) throws IOException {
            boolean ack = true;
            if (length != 8) {
                throw Http20Draft14.ioException("TYPE_PING length != 8: %s", Integer.valueOf(length));
            } else if (streamId != 0) {
        ...
        private void readGoAway(Handler handler, int length, byte flags, int streamId) throws IOException {
            if (length < 8) {
                throw Http20Draft14.ioException("TYPE_GOAWAY length < 8: %s", Integer.valueOf(length));
            } else if (streamId != 0) {
                ...
                ErrorCode errorCode = ErrorCode.fromHttp2(this.source.readInt());
                if (errorCode == null) {
                    throw Http20Draft14.ioException("TYPE_GOAWAY unexpected error code: %d", Integer.valueOf(errorCodeInt));
                }
        ...
        private void readWindowUpdate(Handler handler, int length, byte flags, int streamId) throws IOException {
            if (length != 4) {
                throw Http20Draft14.ioException("TYPE_WINDOW_UPDATE length !=4: %s", Integer.valueOf(length));
            }
            long increment = ((long) this.source.readInt()) & 2147483647L;
            if (increment == 0) {
                throw Http20Draft14.ioException("windowSizeIncrement was 0", Long.valueOf(increment));
            } else {
    ...
    static final class Writer implements FrameWriter {
        ...
        public synchronized void connectionPreface() throws IOException {
            if (this.closed) {
                ...
                this.sink.write(Http20Draft14.CONNECTION_PREFACE.toByteArray());
                this.sink.flush();
            }
        }
        ...
        public synchronized void pushPromise(int streamId, int promisedStreamId, List<Header> requestHeaders) throws IOException {
            if (this.closed) {
                ...
                frameHeader(streamId, length + 4, Http20Draft14.TYPE_PUSH_PROMISE, byteCount == ((long) length) ? (byte) 4 : (byte) 0);
        ...
        private void writeContinuationFrames(int streamId, long byteCount) throws IOException {
            while (byteCount > 0) {
                ...
                frameHeader(streamId, length, Http20Draft14.TYPE_CONTINUATION, byteCount == 0 ? (byte) 4 : (byte) 0);
                this.sink.write(this.hpackBuffer, (long) length);
            }
        }
        ...
        public synchronized void ping(boolean ack, int payload1, int payload2) throws IOException {
            ...
            frameHeader(0, 8, Http20Draft14.TYPE_PING, ack ? (byte) 1 : (byte) 0);
        ...
        public synchronized void goAway(int lastGoodStreamId, ErrorCode errorCode, byte[] debugData) throws IOException {
            if (this.closed) {
                ...
                frameHeader(0, debugData.length + 8, Http20Draft14.TYPE_GOAWAY, (byte) 0);
        ...
        public synchronized void windowUpdate(int streamId, long windowSizeIncrement) throws IOException {
            if (this.closed) {
                ...
                throw Http20Draft14.illegalArgument("windowSizeIncrement == 0 || windowSizeIncrement > 0x7fffffffL: %s", Long.valueOf(windowSizeIncrement));
            } else {
        ...
        void frameHeader(int streamId, int length, byte type, byte flags) throws IOException {
            if (Internal.logger.isLoggable(Level.FINE)) {
                Internal.logger.fine(FrameLogger.formatHeader(false, streamId, length, type, flags));
            }
            if (length > this.maxFrameSize) {
                throw Http20Draft14.illegalArgument("FRAME_SIZE_ERROR length > %d: %d", Integer.valueOf(this.maxFrameSize), Integer.valueOf(length));
            } else if ((ExploreByTouchHelper.INVALID_ID & streamId) != 0) {
                throw Http20Draft14.illegalArgument("reserved bit set: %s", Integer.valueOf(streamId));
            } else {
                Http20Draft14.writeMedium(this.sink, length);
