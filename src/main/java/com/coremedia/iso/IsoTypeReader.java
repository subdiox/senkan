...
public final class IsoTypeReader {
    ...
    public static double readFixedPoint1616(ByteBuffer bb) {
        ...
        bb.get(bytes);
        return ((double) ((((0 | ((bytes[0] << 24) & ViewCompat.MEASURED_STATE_MASK)) | ((bytes[1] << 16) & 16711680)) | ((bytes[2] << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK)) | (bytes[3] & 255))) / 65536.0d;
    }

    public static double readFixedPoint0230(ByteBuffer bb) {
        byte[] bytes = new byte[4];
        bb.get(bytes);
        return ((double) ((((0 | ((bytes[0] << 24) & ViewCompat.MEASURED_STATE_MASK)) | ((bytes[1] << 16) & 16711680)) | ((bytes[2] << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK)) | (bytes[3] & 255))) / 1.073741824E9d;
    }
    ...
    public static String read4cc(ByteBuffer bb) {
        ...
        bb.get(codeBytes);
        int result = ((((codeBytes[0] << 24) & ViewCompat.MEASURED_STATE_MASK) | ((codeBytes[1] << 16) & 16711680)) | ((codeBytes[2] << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK)) | (codeBytes[3] & 255);
