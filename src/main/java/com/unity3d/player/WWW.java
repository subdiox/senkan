...
class WWW extends Thread {
    ...
    public void runSafe() {
        ...
        try {
            ...
            if (!url.getProtocol().equalsIgnoreCase(Key.FILE) || url.getHost() == null || url.getHost().length() == 0) {
                int min;
                ...
                i = (url.getProtocol().equalsIgnoreCase(Key.FILE) || url.getProtocol().equalsIgnoreCase("jar")) ? contentLength == 0 ? 32768 : Math.min(contentLength, 32768) : 1428;
                int i3 = 0;
                try {
                    String str;
