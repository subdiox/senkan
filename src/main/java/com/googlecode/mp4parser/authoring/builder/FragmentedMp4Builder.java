...
public class FragmentedMp4Builder implements Mp4Builder {
    ...
    protected void createSaio(long startSample, long endSample, CencEncyprtedTrack track, int sequenceNumber, TrackFragmentBox parent) {
        ...
        offset += 16;
        for (Box box2 : ((MovieFragmentBox) parent.getParent()).getBoxes()) {
            if (box2 == parent) {
                break;
            }
            offset += box2.getSize();
    ...
    protected Box createEdts(Track track, Movie movie) {
        ...
        for (Edit edit : track.getEdits()) {
            entries.add(new EditListBox.Entry(elst, ((long) edit.getSegmentDuration()) * movie.getTimescale(), (edit.getMediaTime() * track.getTrackMetaData().getTimescale()) / edit.getTimeScale(), edit.getMediaRate()));
        }
