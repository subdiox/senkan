...
public class BitmapHelper {
    ...
    public static Bitmap getBitmap(String path, int inSampleSize) throws Throwable {
        return TextUtils.isEmpty(path) ? null : getBitmap(new File(path), inSampleSize);
    }
    ...
    public static CompressFormat getBmpFormat(String filePath) {
        ...
        return (mime.endsWith("png") || mime.endsWith("gif")) ? CompressFormat.PNG : CompressFormat.JPEG;
    }
    ...
    public static CompressFormat getBmpFormat(byte[] data) {
        ...
        if (mime != null) {
            return (mime.endsWith("png") || mime.endsWith("gif")) ? CompressFormat.PNG : format;
        } else {
            return format;
        }
    }
    ...
    private static String getFileName(HttpConnection conn, String url) throws Throwable {
        ...
        return (dot <= 0 || lastPart.length() - dot >= 10) ? name : name + lastPart.substring(dot);
    }
    ...
    private static String getMime(byte[] bytes) {
        ...
        return (bytesStartWith(bytes, new byte[]{(byte) 73, (byte) 73, (byte) 42}) || bytesStartWith(bytes, new byte[]{(byte) 77, (byte) 77, (byte) 42})) ? "tif" : null;
    }
