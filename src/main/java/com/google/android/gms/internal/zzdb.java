...
public class zzdb extends Thread {
    ...
    public void run() {
        while (true) {
            try {
                ...
                zzpe.zzb("Error in ContentFetchTask", e2);
        ...
        try {
            if (!TextUtils.isEmpty(str)) {
                ...
                if (TextUtils.isEmpty(webView.getTitle())) {
                    ...
                    com_google_android_gms_internal_zzcy.zza(new StringBuilder((String.valueOf(valueOf).length() + 1) + String.valueOf(optString).length()).append(valueOf).append(IOUtils.LINE_SEPARATOR_UNIX).append(optString).toString(), z, webView.getX(), webView.getY(), (float) webView.getWidth(), (float) webView.getHeight());
                }
            }
    ...
    public void zzeh() {
        synchronized (this.zzrN) {
            this.zzxD = true;
            zzpe.zzbc("ContentFetchThread: paused, mPause = " + this.zzxD);
        }
    }
